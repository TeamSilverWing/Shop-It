var shopit = function()
{
	...
	self.viewItem = function(req,res)
	{
		...
		connection.query('SELECT * FROM Item WHERE `Item`.`id` = '+req.params.id,function(err, rows, field)
		{
			...
			{
				user.valid = 1;
				connection.query('SELECT * FROM CartItem WHERE `CartItem`.item = '+
					htmlspecialchars(req.params.id) + ' AND `CartItem`.buyer = '+ses.user_id, function(err1,rows1,field1)
				{
					if(err1==null)
					{
						if(rows1.length == 1)
						{
							rows[0].visibility = 0;
							return res.render('item',{item : rows[0],user : user});
						}	
						else
						{
							rows[0].visibility = 1;
